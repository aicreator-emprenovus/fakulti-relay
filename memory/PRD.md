...
- [x] Mobile responsiveness (2026-05-05):
  - Sidebar `mobileOpen` lifted to `AdminLayout`. Removed the floating hamburger that overlapped content; replaced with a sticky mobile-only top bar (`md:hidden`, h-12) that hosts the menu trigger and brand "Fakulti CRM". Layout closes drawer automatically on route change.
  - Chat page now WhatsApp-like on mobile: `mobileView` state ('list' | 'chat'). On screens <md, only the conversation list shows by default; tapping a session swaps to the chat view, with a back-arrow (←) that returns to the list. Desktop behaviour unchanged (both panels visible).
