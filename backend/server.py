...
async def startup():
    ...
    # === Seed v2 rules (additive) — new commercial flow Apr 2026 ===
    # Marker for idempotency: insert only once based on a unique tag in description.
    v2_marker = await db.automation_rules.find_one({"name": "[v2] Saludo bienvenida Laboratorios Fakulti"}, {"_id": 0, "id": 1})
    if not v2_marker:
        ...
        v2_rules = [
            {"id": str(uuid.uuid4()), "name": "[v2] Saludo bienvenida Laboratorios Fakulti", "trigger_type": "nuevo_lead", "trigger_value": "", "action_type": "respuesta_ia", "action_value": "Envia textualmente: ¡Hola! 👋 Te damos la bienvenida a Laboratorios Fakulti®. Cuentame ¿desde que ciudad nos escribes?", "description": "Saludo oficial. Reemplaza al saludo viejo.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Pregunta inicial: ciudad", "trigger_type": "lead_sin_datos", "trigger_value": "city", "action_type": "respuesta_ia", "action_value": "Solicita SOLO la ciudad si aun no la dio. No pidas nombre, direccion ni email aqui.", "description": "Solo se pide la ciudad al inicio.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Respuesta de precios textual", "trigger_type": "intencion_ia", "trigger_value": "precio,cuanto cuesta,valor,promocion,promo,presentacion,sachet,bolsa,doypack,caja", "action_type": "respuesta_ia", "action_value": "Responde TEXTUALMENTE con prices_response del bot_config del producto. NUNCA inventes precios. NUNCA cites $55 ni $59.", "description": "Bot debe usar prices_response y nunca el precio base $55.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Respuesta de sabor textual", "trigger_type": "intencion_ia", "trigger_value": "sabor,a que sabe,sabor neutro,gusto,rico,dulce,sal", "action_type": "respuesta_ia", "action_value": "Responde TEXTUALMENTE con flavor_response del bot_config: 'nuestro Caldo de Huesos Hidrolizado Fakulti tiene sabor neutro gracias a los procesos biotecnologicos...'", "description": "Respuesta exacta para preguntas de sabor.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Politica de envio (>=$35 gratis / <$35 = $4)", "trigger_type": "intencion_ia", "trigger_value": "envio,domicilio,delivery,entrega,cuanto cuesta el envio,gratis", "action_type": "respuesta_ia", "action_value": "Politica: envio gratis si compra >= $35 USD. Si < $35 USD, costo de envio $4 USD. Algunas promos ya incluyen envio gratis.", "description": "Reglas de envio comerciales actualizadas.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Calculo de total con cantidad y envio", "trigger_type": "analisis_conversacion", "trigger_value": "cantidad_confirmada", "action_type": "respuesta_ia", "action_value": "Cuando el cliente confirme cantidad, calcula el subtotal con prices_response y agrega $4 si subtotal < $35. Confirma: 'Son X unidades por $Y en total (incluye envio si aplica).'", "description": "Calculo automatico del total.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Datos de transferencia PRODUBANCO (textual)", "trigger_type": "analisis_conversacion", "trigger_value": "total_confirmado", "action_type": "respuesta_ia", "action_value": "Envia TEXTUALMENTE el bloque deposit_info del bot_config (PRODUBANCO, HEALTHY COMMUNITY HEALTHAFFILIATE S.A.S, Cuenta Corriente 02005341538, RUC 0591759868001, Correo cobros@hycinternacional.com).", "description": "Datos bancarios para deposito.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Mensaje post-pago (direccion + facturacion)", "trigger_type": "analisis_conversacion", "trigger_value": "deposito_compartido", "action_type": "respuesta_ia", "action_value": "INMEDIATAMENTE despues de los datos de transferencia, envia post_payment_data_request: 'Una vez realizado el pago, comparteme el comprobante y los datos: DIRECCION (Sector, Calle principal, Calle secundaria, Numeracion, Referencia del sector, Referencia del domicilio) y DATOS FACTURACION (Nombre, CI o RUC, Correo).'", "description": "Unico momento donde se piden direccion + facturacion.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] PROHIBIDO pedir direccion antes del pago", "trigger_type": "comportamiento_bot", "trigger_value": "no_pedir_datos_antes_pago", "action_type": "restriccion", "action_value": "PROHIBIDO pedir direccion, sector, calle, CI, RUC, correo o datos de facturacion antes de compartir los datos de transferencia. Solo se solicitan en el mensaje post-pago.", "description": "Restriccion absoluta del flujo.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] PROHIBIDO entregas parciales", "trigger_type": "comportamiento_bot", "trigger_value": "entrega_unica", "action_type": "restriccion", "action_value": "TODA compra se envia en UNA SOLA ENTREGA. PROHIBIDO preguntar si la entrega es en partes o sugerir fraccionar.", "description": "Logistica simple.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] PROHIBIDO inventar cantidades", "trigger_type": "comportamiento_bot", "trigger_value": "no_inventar_cantidades", "action_type": "restriccion", "action_value": "PROHIBIDO ofrecer cantidades inventadas tipo '1, 2 o 1000 cajas'. Si el cliente dio cantidad, usala exacta. Si no, pregunta abierto.", "description": "Anti-alucinacion de cantidades.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Producto siempre Bone Broth Hidrolizado", "trigger_type": "comportamiento_bot", "trigger_value": "nombre_correcto_producto", "action_type": "restriccion", "action_value": "El producto se llama Bone Broth Hidrolizado o Caldo de Huesos Hidrolizado Fakulti. PROHIBIDO usar 'Bombro' u otra variante.", "description": "Nombre correcto del producto.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Confirmacion final post-comprobante", "trigger_type": "analisis_conversacion", "trigger_value": "comprobante_recibido", "action_type": "respuesta_ia", "action_value": "Cuando el cliente envie el comprobante + direccion + datos de facturacion, agradece, confirma que se procesara el pedido y avisa que recibira confirmacion de envio en breve.", "description": "Cierre de la venta.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Maximo 1 mensaje por respuesta", "trigger_type": "comportamiento_bot", "trigger_value": "un_mensaje_por_turno", "action_type": "restriccion", "action_value": "UN SOLO MENSAJE por respuesta del bot. NUNCA dividir en 2 bloques. NUNCA enviar dos mensajes seguidos para el mismo input.", "description": "Bot conciso, no spam.", "active": True},
            {"id": str(uuid.uuid4()), "name": "[v2] Sin re-saludo en conversaciones activas", "trigger_type": "comportamiento_bot", "trigger_value": "no_resaludar", "action_type": "restriccion", "action_value": "Si el cliente ya fue saludado o ya dio su ciudad, NO repitas el saludo. Continua el flujo donde se quedo.", "description": "Conversacion natural.", "active": True},
        ]
