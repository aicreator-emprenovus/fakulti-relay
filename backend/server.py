...
@api_router.post("/automation/run-now")
async def run_automation_now(user=Depends(get_current_user)):
    """Manually trigger automation rules processing (admin only)."""
    ...
    wa_ready = wa_config and wa_config.get("phone_number_id") and wa_config.get("access_token")
    if not wa_ready:
        raise HTTPException(status_code=400, detail="WhatsApp no está configurado")
            ...
            for lead in leads:
                ...
                if action_type == "enviar_mensaje":
                    ...
                    if wa_sent:
                        ...
                        total_failed += 1
                        details.append(f"{lead.get('name', phone)}: {rule['name']} - Falló (24h window / template)")
    ...
    return {
        "message": f"Automatización ejecutada: {total_sent} enviados, {total_failed} fallidos, {total_actions} cambios de etapa",
        ...
        try:
            ...
            for rule in rules:
                try:
                    hours = int(rule.get("trigger_value", "24"))
                    cutoff = (now - timedelta(hours=hours)).isoformat()
                    # Next threshold: find the NEXT rule's hours to avoid re-sending
                    max_hours = hours * 2  # Don't send if they're past double the time (another rule handles them)
                    upper_cutoff = (now - timedelta(hours=max_hours)).isoformat() if max_hours < 200 else None
                    
                    query = {
                        "last_interaction": {"$lte": cutoff},
                        "whatsapp": {"$ne": ""},
                        "funnel_stage": {"$nin": ["perdido", "cliente_activo"]},  # Skip lost & active customers
                    }
                    if upper_cutoff:
                        query["last_interaction"]["$gte"] = upper_cutoff
                    
                    # Check which leads haven't received this specific reminder already
                    ...
                    for lead in leads:
                        ...
                        # Check if we already sent this rule's reminder to this lead
                        already_sent = await db.automation_log.find_one({
                            "lead_id": lead_id,
                            "rule_id": rule["id"],
                            "sent_at": {"$gte": cutoff}  # Only within this time window
                        }, {"_id": 0})
                        if already_sent:
                            continue
                        ...
                        # Log that we processed this rule for this lead
                        await db.automation_log.insert_one({
                            "lead_id": lead_id,
                            "rule_id": rule["id"],
                            "rule_name": rule.get("name", ""),
...
async def startup():
    ...
    asyncio.create_task(process_automation_rules())
